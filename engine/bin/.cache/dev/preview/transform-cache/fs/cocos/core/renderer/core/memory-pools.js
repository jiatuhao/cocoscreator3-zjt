    ...
    execute: function () {
      ...
      BufferPool = /*#__PURE__*/function () {
        ...
        _proto.get = function get(handle, element) {
          ...
          if (DEBUG && (!handle || chunk < 0 || chunk >= bufferViews.length || entry < 0 || entry >= this._entriesPerChunk || this._freelists[chunk].find(function (n) {
            return n === entry;
          }))) {
        ...
        _proto.free = function free(handle) {
          ...
          if (DEBUG && (!handle || chunk < 0 || chunk >= this._freelists.length || entry < 0 || entry >= this._entriesPerChunk || this._freelists[chunk].find(function (n) {
            return n === entry;
          }))) {
            console.warn('invalid buffer pool handle');
            return;
      ...
      _export("ObjectPool", ObjectPool = /*#__PURE__*/function () {
        ...
        _proto2.get = function get(handle) {
          ...
          if (DEBUG && (!handle || index < 0 || index >= this._array.length || this._freelist.find(function (n) {
            return n === index;
          }))) {
      ...
      nodeViewDataType = (_nodeViewDataType = {}, _nodeViewDataType[NodeView.FLAGS_CHANGED] = BufferDataType.UINT32, _nodeViewDataType[NodeView.LAYER] = BufferDataType.UINT32, _nodeViewDataType[NodeView.WORLD_SCALE] = BufferDataType.FLOAT32, _nodeViewDataType[NodeView.WORLD_POSITION] = BufferDataType.FLOAT32, _nodeViewDataType[NodeView.WORLD_ROTATION] = BufferDataType.FLOAT32, _nodeViewDataType[NodeView.WORLD_MATRIX] = BufferDataType.FLOAT32, _nodeViewDataType[NodeView.COUNT] = BufferDataType.NEVER, _nodeViewDataType); // @ts-expect-error Don't alloc memory for Vec3, Quat, Mat4 on web, as they are accessed by class member variable.
