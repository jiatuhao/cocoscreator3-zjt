System.register("q-bundled:///fs/cocos/ui/layout.js", ["../core/data/decorators/index.js", "../core/components/component.js", "../core/math/index.js", "../core/value-types/enum.js", "../2d/framework/ui-transform.js", "../core/platform/event-manager/event-enum.js", "../core/director.js", "../core/scene-graph/node-enum.js", "../core/index.js"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _export("Layout", Layout = (_dec = ccclass('cc.Layout'), _dec2 = help('i18n:cc.Layout'), _dec3 = executionOrder(110), _dec4 = menu('UI/Layout'), _dec5 = requireComponent(UITransform), _dec6 = visible(function () {
      ...
      }), _dec7 = tooltip('自动对齐。在 Type 为 Horizontal 或 Vertical 时按同个方向位置排列'), _dec8 = visible(function () {
        return this._layoutType === Type.VERTICAL;
      }), _dec9 = tooltip('自动对齐。在 Type 为 Vertical 时按同个方向位置排列'), _dec10 = type(Type), _dec11 = tooltip('自动布局模式，包括：\n 1. NONE，不会对子节点进行自动布局 \n 2. HORIZONTAL，横向自动排布子物体 \n 3. VERTICAL，垂直自动排布子物体\n 4. GRID, 采用网格方式对子物体自动进行布局'), _dec12 = type(ResizeMode), _dec13 = visible(function () {
        return this._layoutType !== Type.NONE;
      }), _dec14 = tooltip('缩放模式，包括：\n 1. NONE，不会对子节点和容器进行大小缩放 \n 2. CONTAINER, 对容器的大小进行缩放 \n 3. CHILDREN, 对子节点的大小进行缩放'), _dec15 = visible(function () {
        if (this.type === Type.GRID && this._resizeMode === ResizeMode.CHILDREN) {
          return true;
        }

        return false;
      }), _dec16 = tooltip('每个格子的大小，只有布局类型为 GRID 的时候才有效'), _dec17 = type(AxisDirection), _dec18 = tooltip('起始轴方向类型，可进行水平和垂直布局排列，只有布局类型为 GRID 的时候才有效'), _dec19 = tooltip('容器内左边距，只会在一个布局方向上生效'), _dec20 = tooltip('容器内右边距，只会在一个布局方向上生效'), _dec21 = tooltip('容器内上边距，只会在一个布局方向上生效'), _dec22 = tooltip('容器内下边距，只会在一个布局方向上生效'), _dec23 = tooltip('子节点之间的水平间距'), _dec24 = tooltip('子节点之间的垂直间距'), _dec25 = type(VerticalDirection), _dec26 = tooltip('垂直排列子节点的方向'), _dec27 = type(HorizontalDirection), _dec28 = tooltip('水平排列子节点的方向'), _dec29 = type(Constraint), _dec30 = visible(function () {
        return this.type === Type.GRID;
      }), _dec31 = tooltip('容器内布局约束'), _dec32 = visible(function () {
        return this._constraint !== Constraint.NONE;
      }), _dec33 = tooltip('容器内布局约束数值'), _dec34 = tooltip('子节点缩放比例是否影响布局'), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = executeInEditMode(_class = (_class2 = (_temp = _class3 = /*#__PURE__*/function (_Component) {
        ...
        _proto._getFixedColumn = function _getFixedColumn() {
          ...
          var num = this._constraint === Constraint.FIXED_ROW ? Math.round(this._usefulLayoutObj.length / this._constraintNum) : this._constraintNum; // Horizontal sorting always counts the number of columns
