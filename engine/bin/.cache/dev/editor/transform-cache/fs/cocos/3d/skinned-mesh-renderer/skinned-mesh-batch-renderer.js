...
const batch_id = new _index2.Attribute(_define.AttributeName.ATTR_BATCH_ID, _define.Format.R32F);
const batch_uv = new _index2.Attribute(_define.AttributeName.ATTR_BATCH_UV, _define.Format.RG32F);
const batch_extras_size = _define.FormatInfos[batch_id.format].size + _define.FormatInfos[batch_uv.format].size;
...
let SkinnedMeshBatchRenderer = (_dec6 = (0, _index.ccclass)('cc.SkinnedMeshBatchRenderer'), _dec7 = (0, _index.help)('i18n:cc.SkinnedMeshBatchRenderer'), _dec8 = (0, _index.executionOrder)(100), _dec9 = (0, _index.menu)('Mesh/SkinnedMeshBatchRenderer'), _dec10 = (0, _index.tooltip)('i18n:batched_skinning_model.atlas_size'), _dec11 = (0, _index.type)([_attribute.CCString]), _dec12 = (0, _index.tooltip)('i18n:batched_skinning_model.batchable_texture_names'), _dec13 = (0, _index.type)([SkinnedMeshUnit]), _dec14 = (0, _index.tooltip)('i18n:batched_skinning_model.units'), _dec15 = (0, _index.visible)(false), _dec16 = (0, _index.visible)(false), _dec6(_class4 = _dec7(_class4 = _dec8(_class4 = (0, _index.executeInEditMode)(_class4 = _dec9(_class4 = (_class5 = (_temp2 = class SkinnedMeshBatchRenderer extends _skinnedMeshRenderer.SkinnedMeshRenderer {
  ...
  cookMaterials() {
    ...
    for (let i = 0; i < tech.passes.length; i++) {
      ...
      for (const prop in pass.properties) {
        if (pass.properties[prop].type >= _define.Type.SAMPLER1D) {
          // samplers
          let tex = null;
  ...
  cookMeshes() {
    ...
    let posOffset = 0;
    let posFormat = _define.Format.UNKNOWN;
    let normalOffset = 0;
    let normalFormat = _define.Format.UNKNOWN;
    let tangentOffset = 0;
    let tangentFormat = _define.Format.UNKNOWN;
    let uvOffset = 0;
    let uvFormat = _define.Format.UNKNOWN;
    let jointOffset = 0;
    let jointFormat = _define.Format.UNKNOWN; // prepare joint index map
    ...
    for (let i = 0; i < unitLen; i++) {
      ...
      for (let b = 0; b < newMesh.struct.vertexBundles.length; b++) {
        ...
        posFormat = _define.Format.UNKNOWN;

        for (let a = 0; a < bundle.attributes.length; a++) {
          const attr = bundle.attributes[a];

          if (attr.name === _define.AttributeName.ATTR_POSITION) {
            posFormat = attr.format;
            break;
          }

          posOffset += _define.FormatInfos[attr.format].size;
        }
        ...
        normalFormat = _define.Format.UNKNOWN;

        for (let a = 0; a < bundle.attributes.length; a++) {
          const attr = bundle.attributes[a];

          if (attr.name === _define.AttributeName.ATTR_NORMAL) {
            normalFormat = attr.format;
            break;
          }

          normalOffset += _define.FormatInfos[attr.format].size;
        }
        ...
        tangentFormat = _define.Format.UNKNOWN;

        for (let a = 0; a < bundle.attributes.length; a++) {
          const attr = bundle.attributes[a];

          if (attr.name === _define.AttributeName.ATTR_TANGENT) {
            tangentFormat = attr.format;
            break;
          }

          tangentOffset += _define.FormatInfos[attr.format].size;
        }
        ...
        uvFormat = _define.Format.UNKNOWN;

        for (let a = 0; a < bundle.attributes.length; a++) {
          const attr = bundle.attributes[a];

          if (attr.name === _define.AttributeName.ATTR_BATCH_UV) {
            uvFormat = attr.format;
            break;
          }

          uvOffset += _define.FormatInfos[attr.format].size;
        }
        ...
        jointFormat = _define.Format.UNKNOWN;

        for (let a = 0; a < bundle.attributes.length; a++) {
          const attr = bundle.attributes[a];

          if (attr.name === _define.AttributeName.ATTR_JOINTS) {
            jointFormat = attr.format;
            break;
          }

          jointOffset += _define.FormatInfos[attr.format].size;
        }
    ...
    for (let p = 0; p < mesh.struct.primitives.length; p++) {
      ...
      for (; bundleIdx < primitive.vertexBundelIndices.length; bundleIdx++) {
        ...
        for (let a = 0; a < bundle.attributes.length; a++) {
          ...
          if (attr.name === _define.AttributeName.ATTR_TEX_COORD) {
            uvFormat = attr.format;
            break;
          }
