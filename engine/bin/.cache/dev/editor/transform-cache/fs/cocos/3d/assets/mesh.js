...
let Mesh = (_dec = (0, _index.ccclass)('cc.Mesh'), _dec(_class = (_class2 = (_temp = class Mesh extends _asset.Asset {
  ...
  initialize() {
    ...
    for (let i = 0; i < this._struct.primitives.length; i++) {
      ...
      if (prim.indexView) {
        ...
        if (dstStride === 4 && !gfxDevice.hasFeature(_define.Feature.ELEMENT_INDEX_UINT)) {
        ...
        indexBuffer = gfxDevice.createBuffer(new _index4.BufferInfo(_define.BufferUsageBit.INDEX, _define.MemoryUsageBit.DEVICE, dstSize, dstStride));
  ...
  getBoneSpaceBounds(skeleton) {
    ...
    for (let p = 0; p < primitives.length; p++) {
      const joints = this.readAttribute(p, _define.AttributeName.ATTR_JOINTS);
      const weights = this.readAttribute(p, _define.AttributeName.ATTR_WEIGHTS);
      const positions = this.readAttribute(p, _define.AttributeName.ATTR_POSITION);
      ...
      if (worldMatrix) {
        ...
        for (let i = 0; i < struct.vertexBundles.length; i++) {
          ...
          for (let j = 0; j < vtxBdl.attributes.length; j++) {
            if (vtxBdl.attributes[j].name === _define.AttributeName.ATTR_POSITION || vtxBdl.attributes[j].name === _define.AttributeName.ATTR_NORMAL) {
              const {
                format
              } = vtxBdl.attributes[j];
              ...
              for (let vtxIdx = 0; vtxIdx < vertexCount; vtxIdx++) {
                ...
                switch (vtxBdl.attributes[j].name) {
                  case _define.AttributeName.ATTR_POSITION:
                    vec3_temp.transformMat4(worldMatrix);
                    break;

                  case _define.AttributeName.ATTR_NORMAL:
    ...
    for (let i = 0; i < this._struct.vertexBundles.length; ++i) {
      ...
      for (const attr of bundle.attributes) {
        ...
        for (const dstAttr of dstBundle.attributes) {
          ...
          dstVBOffset += _define.FormatInfos[dstAttr.format].size;
        }

        if (hasAttr) {
          attrSize = _define.FormatInfos[attr.format].size;
          ...
          for (let v = 0; v < dstBundle.view.count; ++v) {
            ...
            if ((attr.name === _define.AttributeName.ATTR_POSITION || attr.name === _define.AttributeName.ATTR_NORMAL) && worldMatrix) {
        ...
        srcAttrOffset += _define.FormatInfos[attr.format].size;
      }
  ...
  readAttribute(primitiveIndex, attributeName) {
    ...
    this._accessAttribute(primitiveIndex, attributeName, (vertexBundle, iAttribute) => {
      ...
      const storageConstructor = (0, _define.getTypedArrayConstructor)(_define.FormatInfos[format]);
      ...
      const formatInfo = _define.FormatInfos[format];
      const reader = getReader(inputView, format);
  ...
  copyIndices(primitiveIndex, outputArray) {
    ...
    const indexFormat = primitive.indexView.stride === 1 ? _define.Format.R8UI : primitive.indexView.stride === 2 ? _define.Format.R16UI : _define.Format.R32UI;
    const reader = getReader(new DataView(this._data.buffer), indexFormat);

    for (let i = 0; i < indexCount; ++i) {
      outputArray[i] = reader(primitive.indexView.offset + _define.FormatInfos[indexFormat].size * i);
    }
  ...
  _createVertexBuffers(gfxDevice, data) {
    return this._struct.vertexBundles.map(vertexBundle => {
      const vertexBuffer = gfxDevice.createBuffer(new _index4.BufferInfo(_define.BufferUsageBit.VERTEX, _define.MemoryUsageBit.DEVICE, vertexBundle.view.length, vertexBundle.view.stride));
...
function getOffset(attributes, attributeIndex) {
  ...
  for (let i = 0; i < attributeIndex; ++i) {
    ...
    result += _define.FormatInfos[attribute.format].size;
  }
...
function getComponentByteLength(format) {
  const info = _define.FormatInfos[format];
